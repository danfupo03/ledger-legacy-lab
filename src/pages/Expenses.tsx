...
export default function Expenses() {
  ...
  const filteredExpenses = expenses
    .filter((e) => {
      if (
        ...
        return false;
      if (filters.category !== "all" && e.categoryId !== filters.category)
        return false;
      if (filters.account !== "all" && e.accountId !== filters.account)
        return false;
      if (filters.budget !== "all") {
        if (filters.budget === "none" && e.budgetId) return false;
        if (filters.budget !== "none" && e.budgetId !== filters.budget)
          return false;
      }
  ...
  useEffect(() => {
    document.title = "Expenses — Personal Finance";
  }, []);
        ...
        <Dialog open={editOpen} onOpenChange={setEditOpen}>
          <DialogContent>
            ...
            <div className="grid gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-expense-budget">Budget (Optional)</Label>
                <Select
                  value={editForm.budgetId || "none"}
                  onValueChange={(v) =>
                    setEditForm((f) => ({
                      ...f,
                      budgetId: v === "none" ? null : v,
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  type="date"
                  value={format(new Date(editForm.date), "yyyy-MM-dd")}
                  onChange={(e) =>
                    setEditForm((f) => ({
                      ...f,
                      date: new Date(e.target.value).toISOString(),
              ...
              <Button
                onClick={() => {
                  if (editingId) updateExpense(editingId, editForm);
                  setEditOpen(false);
                  setEditingId(null);
                }}
              >
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filteredExpenses.map((e) => {
                  ...
                  const budget = e.budgetId
                    ? budgets.find((b) => b.id === e.budgetId)
                    : null;
                  return (
                    <tr key={e.id} className="border-b">
                      ...
                      <td className="py-2">
                        {e.amount.toFixed(2)} {acc.currency}
                      </td>
